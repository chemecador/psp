...
public class Servidor {
    ...
    private void gestionar() {
        try {
            ...
            //se lo enviamos al jugador 1, cifrado con la librería Cifrado
            out1.writeUTF(c.cifrar(json));

            //idem
            tablero.setT("2");
            json = gson.toJson(tablero);
            out2.writeUTF(c.cifrar(json));
            ...
            while (r == 0) {
                ...
                if (turno == 1) {
                    ...
                    t = c.descifrar(in1.readUTF());
                    tablero = gson.fromJson(t,Tablero.class);
                    t = tablero.getT();
                    //cambio de turno
                    turno++;
                } else if (turno == 2) {
                    t = c.descifrar(in2.readUTF());
                    tablero = gson.fromJson(t,Tablero.class);
                    t = tablero.getT();
                    turno--;
                } else {
    ...
    private void enviarATodos(String s) throws IOException {
        ...
        for (Socket soc : jugadores) {
            ...
            tablero.setT(s);
            json = gson.toJson(tablero);
            //se envía el paquete a cada cliente a través del flujo, cifrado con la librería Cifrado
    ...
    private void resultado(int r) {
        ...
        if (r == 1) {
            ...
            try {
                tablero.setT("W");
                json = gson.toJson(tablero);
                //enviamos el tablero en json, cifrado con la librería Cifrado
                out1.writeUTF(c.cifrar(json));
                tablero.setT("L");
                json = gson.toJson(tablero);
                //enviamos el tablero en json, cifrado con la librería Cifrado
            ...
            try {
                tablero.setT("L");
                json = gson.toJson(tablero);
                //enviamos el resultado en json, cifrado con la librería Cifrado
                out1.writeUTF(c.cifrar(json));
                tablero.setT("W");
                json = gson.toJson(tablero);
                //enviamos el resultado en json, cifrado con la librería Cifrado
