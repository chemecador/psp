...
public class Cliente extends Vista implements ActionListener, Runnable {
    ...
    private Cliente() {
        try {
            ...
            tablero = new Tablero(); //inicializamos el tablero
    ...
    public void run() {
        try {
            while (true) {
                //leo el String que manda el servidor y lo descifro con la librería Cifrado
    ...
    private boolean jugar(int index) {
        ...
        try {
            ...
            // enviamos el tablero dentro de un json, cifrado con la librería Cifrado
    ...
    private void iniciar() {
        ...
        try {
            ...
            //leemos la orden de jugar, descifrada con la librería Cifrado
            s = c.descifrar(in.readUTF());
            tablero = gson.fromJson(s, Tablero.class);
            s = tablero.getT();
            //recibimos el id, descifrado con la librería Cifrado
            s = c.descifrar(in.readUTF());
            tablero = gson.fromJson(s, Tablero.class);
            s = tablero.getT();
            //lo convertimos a char
            this.id = s.charAt(0);
            //leemos el turno, descifrado con la librería Cifrado
            s = c.descifrar(in.readUTF());
            tablero = gson.fromJson(s, Tablero.class);
            s = tablero.getT();
        } catch (IOException e) {
